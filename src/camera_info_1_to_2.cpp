...
void CameraInfoCallback(boost::shared_ptr<sensor_msgs::CameraInfo> ros1_msg)
{
  ...
  std::array<double, 9> k_data, r_data;
  std::array<double, 12> p_data;
  int i = 0, j = 0, k = 0;
  
  for (auto & data : ros1_msg->K)
  {
    k_data[i] = data;
    i++;
  }
...
int main(int argc, char * argv[])
{
  ...
  pub = node->create_publisher<sensor_msgs::msg::CameraInfo>("output", 1);

  // ROS 1 node and subscriber
  ros::init(argc, argv, "camera_info_1_to_2");
  ros::NodeHandle n;
  ros::Subscriber sub = n.subscribe("input", 1, CameraInfoCallback);
